...
\begin{itemize}
    \item \textbf{Baseline Model:} Random Forest Classifier. It is a reliable machine learning model that can handle ojutliers and mixed data types. It also capture non-linear relationships properly with complex tuning.
    ...
    \begin{itemize}
        \item \textit{Preprocessing}: It works by resampling training data in order to balance target distributions across different age groups.
...
\subsection*{Analysis}
The model scores high on predictive metrics but it contains notable disparities. The DP difference of 0.147 and EO difference of 0.224 that there is a demographic group favorable to the model than the other. Historically and contextually, individuals under 25 are usually penalized by credit scoring models, this leads to less credit approvals and higher error rates.
...
\section{AI Chat Use}
The development and refinement of this technical report and the accompanying analysis were supported by the use of AI chat assistants. The complete conversation log detailing the assistance provided can be found at the following link: \url{https://claude.ai/share/5cce4bc4-1baa-4342-8b90-4ca08fd0628f}
